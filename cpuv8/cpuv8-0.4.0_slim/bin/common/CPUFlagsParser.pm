...
$::tools_versions{'CPUFlagsParser.pm'} = 0;
...
sub start_element {
    ...
    if ($elem eq 'flagsdescription') {    # Top-level container
        ...
        || $elem eq 'submit_command'         # OMP2012,CPU2017,CPUv8
        || $elem eq 'fdo_settings'           # OMP2012,CPU2017,CPUv8
        || $elem eq 'sw_environment'         # OMP2012,CPU2017,CPUv8
        || $elem eq 'os_tuning'              # OMP2012,CPU2017,CPUv8
        || $elem eq 'virtual_machine'        # OMP2012,CPU2017,CPUv8
        || $elem eq 'firmware'               # OMP2012,CPU2017,CPUv8
        || $elem eq 'parts'                  # OMP2012,CPU2017,CPUv8
        || $elem eq 'filename'
        || $elem eq 'dumpit'
    ) {
