...
my $http_tiny = $@ eq '';
$::tools_versions{'util_common.pl'} = 0;
...
sub apply_update {
    ...
    push @postapply, 'convert_to_development';
...
sub compress_encode {
    ...
    # The caller may restrict the allowable compression formats.  Bzip2 in particular is a little dangerous when
    # backwards compatibility is needed, as the output from Compress::Bzip2 is different than IO::Compress::Bzip2
    # and older benchmarks assume one or the other.
    my ($input, $formats) = @_;

    # All formats allowed unless the caller specifies.
    my %formats = map { $_ => $_ } (qw(uncompressed gzip bzip2));
    my @tries;
    my $compressed;

    if (reftype($formats) eq 'ARRAY' and @$formats) {
        my %caller_formats = map { $_ => $_ } grep { defined } @formats{@$formats};
        if (%caller_formats) {
            %formats = %caller_formats;
        } else {
            Log(169, "No valid compression types passed. None of (".join(', ', @$formats).") in ("
                .join(', ', sort keys %formats).")\n".Carp::longmess()."\n");
            return undef;
        }
    }

    if (exists($formats{'uncompressed'})) {
        push @tries, ['*', $input];
    }

    if (exists($formats{'gzip'})) {
        $compressed = eval 'Compress::Zlib::compress($input, 9)';
        push @tries, [ '@', $compressed ] unless $@ or !defined($compressed);
    }

    if (exists($formats{'bzip2'})) {
        $compressed = memBzip($input);
        push @tries, [ '&', $compressed ] if defined($compressed);
    }

    if (@tries == 0) {
        Log(169, "No allowable compression types (".join(', ', sort keys %formats).") generated usable output.\n"
            .Carp::longmess()."\n");
        return undef;
    }
...
sub basepeak_munge {
    ...
    if ($bp_only or $::lcsuite =~ /^cpu(2017|v8)/) {
        if ($bp_only) {
