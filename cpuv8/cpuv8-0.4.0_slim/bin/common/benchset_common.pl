...
use strict;
$::tools_versions{'benchset_common.pl'} = 0;
...
sub assemble_units {
    ...
    if (!$peak) {
        $tune = '_base';
    } elsif ($::lcsuite =~ /^(?:mpi2007|omp|accel|cpu(2017|v8))/) {
        # MPI2007, OMP2012, ACCEL, CPU2017, and CPUv8 include '_peak' in the peak
        # metric name
        $tune = '_peak';
    }
...
sub ranks {
    ...
    if ($::lcsuite eq 'omp2012' || $::lcsuite =~ /^cpu(2017|v8)/) {
