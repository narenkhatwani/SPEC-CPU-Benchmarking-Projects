...
$::tools_versions{'vars_common.pl'} = 0;
...
$::suitebase = 'CPU';
$::year = 'v8';
$::release_year = $::year;   # Or just set to $::year
$::suite = $::suitebase.$::year;
$::current_version = '0.4.0'; # UPDATE - version: Adjust by hand
$::treeowner = 'cloyce';    # Mostly for prep_for_release
$::treegroup = 'cpudevel';  # Mostly for prep_for_release
...
# Grab log level from @ARGV if it's there, so early verbose debug output can be output if requested
sub get_default_log_level {
    my ($log_level, @args) = @_;
    my $args = join(' ', @args);
    if ($args =~ /(?:-v|--verbose)[=\s]+(\d+)/) {
        $log_level = $1+0;
    }
    return $log_level;
}
...
$::nonvolatile_config = {
    ...
    'max_report_runs'           => ($::lcsuite =~ /^cpu(2017|v8)/) ? 3 : ($::lcsuite =~ /^(omp|accel)/ ? 3 : 0), # Maximum number of runs to be valid (0 => no max)
    ...
    'min_report_runs'           => ($::lcsuite =~ /^(mpi2007|cpu(2017|v8))$/) ? 2 : 3, # Minimum number of runs to be valid
    ...
    'rawhash_bits'              => ($::lcsuite =~ /^(accel|omp|cpu(2017|v8))/) ? 256 : 128,        # Number of bits in hash algo for overall rawhash (MD5=128; SHA>=256)
...
$::default_config = {
    ...
    'plain_train'               => ($::lcsuite =~ /^cpu(2017|v8)/) ? 1 : 0,    # Allow use of submit for training runs?
...
'strict_rundir_verify'      => 0,
    ...
    'verbose'                   => ::get_default_log_level(5, @ARGV), # Default verbosity level
...
# The "version" for each tools file is its identity, which we express as the hash of its contents.
# We dig though %INC to find the actual path of the file to hash.
sub fixup_tools_versions {
    my (%toolvers) = @_;
    my $bits = $::nonvolatile_config->{'exehash_bits'};

    foreach my $file (grep { length($toolvers{$_}) < 16 } sort keys %toolvers) {
        my $hash = 'NOT_FOUND';
        my @paths = grep { m{/${file}$} } values %INC;
        if (@paths) {
            $hash = ::filedigest($paths[0], $bits);
        }
        $toolvers{$file} = $hash;
    }
