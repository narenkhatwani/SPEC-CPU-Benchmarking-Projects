...
use vars '@ISA';
$::tools_versions{'harness/benchmark.pm'} = 0;
...
sub descmode {
    ...
    push @stuff, 'threads:'.$me->{'threads'} unless ($opts{'no_threads'} or $me->{'threads'} <= 1 or ($::lcsuite =~ /^cpu(2017|v8)/ and $me->runmode =~ /rate$/));
    return join(' ', @stuff);
}
...
sub handle_workload_dir_addition {
    ...
    foreach my $dir (@dirs) {
        if ((::ref_type($dir) eq 'ARRAY')) {
            ...
            my ($bmark, @sizes) = @{$dir};
            if ($bmark eq $me->benchmark) {
                ::Log(0, "ERROR: ".$me->benchmark." is misconfigured to get remote workload directories from itself. ".
                    "Please contact the project leader for ".$me->benchmark." for a fix.\n");
                next;
            }
            if (@sizes > 0) {
                push @sizes, 'all';
...
sub compile_error_result {
    ...
    $result->{'iteration'} = -1;
    $result->{'basepeak'}  = 0;
    $result->{'copies'}    = 1 if $::lcsuite =~ /^cpu(2017|v8)/;
    $result->{'ranks'}     = 1;
    $result->{'threads'}   = 1;
...
sub setup_rundirs {
    ...
    for my $dir (@dirs) {
        $dir->{'bad'} = 1;    # No sums, so always re-create
...
sub make_empty_result {
    ...
    $result->{'copies'}        = $me->copies if $::lcsuite =~ /^cpu(2017|v8)/;
...
sub result_list {
    ...
    if ($::lcsuite =~ /^cpu(2017|v8)/ and defined $copies) {
...
sub setup_run_environment {
    ...
    if (istrue($me->reportable) and $::lcsuite !~ /^cpu(2017|v8)/) {
    ...
    if ($::lcsuite ne 'mpi2007') {
        ...
        if ($::lcsuite =~ /^cpu(2017|v8)/ and $me->runmode =~ /rate$/) {
        ...
        if (defined($threads) && $threads > 0) {
            ...
            if ($::lcsuite =~ /^cpu(2017|v8)/ and $threads == 1) {
