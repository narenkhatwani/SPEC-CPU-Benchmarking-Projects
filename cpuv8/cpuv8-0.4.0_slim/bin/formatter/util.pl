...
$::tools_versions{'formatter/util.pl'} = 0;
...
sub get_basepeak_parallel_info {
    ...
    if ($::lcsuite eq 'mpi2007') {
        $parallel = 'ranks';
            ...
            or ($::lcsuite =~ /^cpu(2017|v8)/ and $config->runmode eq 'speed')) {
        $parallel = 'threads';
    }
...
sub significant_base {
    ...
    my @returnlog_desc = ('return value', 'return order of value', 'return decimal places in value');
    print ("significant(value=\"$value\", min_log=\"$min_log\", "
        ."returnlog=\"$returnlog (".$returnlog_desc[$returnlog+0].")\", hack=\"$hack\")\n") if $debug;
    my ($log);
    ...
    if ($value <= 0) {
        if ($value eq '0' || $value !~ /[\d.]/) {
            ...
            ::Log(0, "NOTICE: Not taking log of negative number ($value); significant_base($value,...) called from\n"
                .Carp::longmess()."\n");
            if ($returnlog) {
                return 0;
            } else {
...
sub get_means {
    ...
    my %means = (
        map {
                ...
                $_ => {
                    ...
                    'units'   => $unit,
                    'dp'      => significant_base($mean, undef, 2, 0, 0),
                }
    ...
    if ($nc) {
        ...
        # "dp" here is misleading, since significant_base() uses this value as the order of the number to determine
        # rounding.  It should never be less than 2 or we start rounding 55.5 to 60 and other bad things.
        $meandp = max(2, map { $means{$_}->{'dp'} } grep { $means{$_}->{'mean'} ne '--' } keys %means);
        foreach my $meanref (values %means) {
            if (::istrue($r->power) or $meanref->{'ispower'} == 0) {
                if ($meanref->{'mean'} =~ /\d/) {
                    $meanref->{'formatted'} = significant_base($meanref->{'mean'}, $meandp, undef, 0, 0);
                } else {
