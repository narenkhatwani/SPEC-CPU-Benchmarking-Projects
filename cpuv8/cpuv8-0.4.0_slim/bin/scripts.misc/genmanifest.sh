...
# Look in each benchmark's Docs directory for the file that prep_for_release
...
find . $novc \( -type f ! -name MANIFEST.tmp -print \) | \
  sed 's#^\./##' | \
  egrep -v '^MANIFEST$' | \
  egrep -v '(\.ignoreme|\.cvsignore|\.DS_Store|\.gitignore|\.gitmodules)' | \
