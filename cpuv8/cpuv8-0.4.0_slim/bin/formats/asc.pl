...
use strict;
$::tools_versions{'formats/asc.pl'} = 0;

require 'util.pl';
require 'flagutils.pl';

our ($name, $extension, $synonyms, $part_of_all, $non_default, $asc_version);
...
sub format {
    ...
    $asc_version = substr($::tools_versions{'formats/asc.pl'}, 0, 8);
    $r->{'table'} = 0 if (@nc);
    ...
    foreach my $line (
        ...
        'Report generated on '.::timeformat('date-time', time)." by $::suite text formatter ($asc_version).",
    ) {
