...
use URI;
use Data::Dumper;
$::tools_versions{'formats/flags.pl'} = 0;

require 'util.pl';
require 'flagutils.pl';

our ($name, $extension, $synonyms, $part_of_all, $non_default, $flags_version);

$name      = 'flags';
$extension = 'flags.html';
$synonyms  = { map { lc($_) => 1 } ($name, 'flag') };
$flags_version = 'unknown';
...
sub format {
    ...
    $flags_version = substr($::tools_versions{'formats/flags.pl'}, 0, 8);
    ...
    push @output, 'Report generated on '.::timeformat('date-time', time)." by SPEC $::suite flags formatter ($flags_version).</p>";
    push @output, '</body>';
    push @output, '</html>';
