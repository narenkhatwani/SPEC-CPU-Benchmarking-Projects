...
$::tools_versions{'formats/ps.pl'} = 0;

require 'util.pl';

our ($name, $extension, $synonyms, $part_of_all, $non_default, $ps_version);
...
sub format () {
    my($me, $r, $path) = @_;
    $ps_version = substr($::tools_versions{'formats/ps.pl'}, 0, 8);
...
sub SPEC_report {
    ...
    $pspdf->Producer("SPEC $::suite PS/PDF formatter ($ps_version)");
...
sub SPEC_table {
    ...
    if ($::lcsuite eq 'mpi2007') {
        ...
        ($rate, $what) = (1, 'threads');
    } elsif (!$rate and $::lcsuite =~ /^cpu(2017|v8)/) {
        ($rate, $what) = (2, 'threads');
    }
...
sub SPEC_trademarks {
    ...
    push @lines, 'Report generated on '.::timeformat('date-time', time)." by $::suite $p->{'mode'} formatter ($ps_version).";
    my @dates;
