...
use IO::File;
use File::Basename;
$::tools_versions{'formats/mail.pl'} = 0;

require 'mailutil.pl';

our ($name, $extension, $synonyms, $part_of_all, $non_default, $debug, $mail_version);
...
sub format {
    my($me, $r, $fn, $files) = @_;
    $mail_version = substr($::tools_versions{'formats/mail.pl'}, 0, 8);
